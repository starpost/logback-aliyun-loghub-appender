package com.sense.cloud.aliyun.loghub.logback;
...
public class LoghubAppender extends AppenderBase<ILoggingEvent> {
    ...
    public int getIoThreadsCount() {
        return config.ioThreadsCount;
    }

    public void setIoThreadsCount(int ioThreadsCount) {
        config.ioThreadsCount = ioThreadsCount;
    }
    ...
    protected void append(ILoggingEvent event) {
        ...
        if (throwable != null) {
            for (StackTraceElementProxy ste : throwable.getStackTraceElementProxyArray()) {
                message += System.lineSeparator() + ste.getSTEAsString();
            }
        }
